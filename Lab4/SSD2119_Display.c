...
static void LCD_GPIOInit(void){
    ...
    // P0, P1, P3, P4 --> 0001 1011 --> 0x1B
    RCGCGPIO |= (1 << 13);                      // activate port P
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    ...
    // Q0, Q2, Q3 --> 1101 --> 0xD
    RCGCGPIO |= (1 << 14);                      // activate port Q
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIODIR_Q |= 0xD;                           // make PQ0, PQ2-3 outputs
    GPIOAFSEL_Q &= ~0xD;                        // disable alternate functions
...
void LCD_PrintFloat(float num){
    ...
    float decimal = (num - integer) * 1000;  // 3 decimal places
    long rounded = (int)decimal;
    if (decimal - rounded >= 0.5) {
      rounded += 1;
    }
    
    LCD_PrintInteger(integer);
    LCD_PrintChar('.');
    //LCD_PrintInteger(rounded);
